...
function getRectangleString(width, height) {
    // First line
    var str_pre = '┌'+'─'.repeat(width - 2)+'┐\n';
    // Common lines
    var str = '│'+' '.repeat(width - 2)+'│\n';
    // End line
    var str_aft = '└'+'─'.repeat(width - 2)+'┘\n';

    return str_pre+str.repeat(height -2 )+str_aft;
}
...
function encodeToRot13(str) {
    var in_arr   = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
    var out_arr  = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
    var i;
    var out_str = '';
    for (i = 0;i < str.length ;i++){
        if (in_arr.indexOf(str[i]) != -1) {
            out_str += out_arr[in_arr.indexOf(str[i])];
        } else  {
            out_str += str[i];
        }
    }
