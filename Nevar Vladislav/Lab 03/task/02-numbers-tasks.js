...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    ...
    let firstVectorLength = Math.sqrt(x1*x1 + y1*y1);
    let secondVectorlength = Math.sqrt(x2*x2+y2*y2);

    return Math.acos(vector/(firstVectorLength*secondVectorlength));
}
 ...
 */
function isPrime(n) {
    for (var i = 2; i < Math.sqrt(n)+0.1;i++)
        if (n % i == 0)
            return false;
    return true;
}
