...
function getRectangleString(width, height) {
    ...
    let bottomSize = '└' + '─'.repeat(width - 2) + '┘\n'
    return topSide + middle + bottomSize
}
...
function encodeToRot13(str) {
    ...
    const bigACode = 'A'.charCodeAt(0) // Code of char 'A'
    const bigZCode = 'Z'.charCodeAt(0) // Code of char 'Z'
    const littleACode = 'a'.charCodeAt(0)
    const littleZCode = 'z'.charCodeAt(0)

    const alphabetLength = bigZCode - bigACode + 1
    ...
    for (let i = 0; i < str.length; i ++) {
       ...
       if (c.match(/[a-z]/i)) {
          ...
          if ((code >= bigACode) && (code <= bigZCode))
             c = String.fromCharCode(((code - bigACode + cipherShift) % alphabetLength) + bigACode)
          else if ((code >= littleACode) && (code <= littleZCode))
             c = String.fromCharCode(((code - littleACode + cipherShift) % alphabetLength) + littleACode)
       }
    output += c
    }
    return output
