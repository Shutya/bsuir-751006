...
function logger(func, logFunc) {
    return function () {
        let argumentsString = JSON.stringify(Array.from(arguments)).slice(1, -1);
        logFunc(func.name + '(' + argumentsString + ') starts');
        let result = func.apply(null, arguments);
        argumentsString = JSON.stringify(Array.from(arguments)).slice(1, -1);
        logFunc(func.name + '(' + argumentsString + ') ends');
        return result;
    }
}
